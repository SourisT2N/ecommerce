$(document).ready(function(){
	...
	$('.join').validate({
		...
		messages: {
			comment: {
				...
				minlength: 'Độ Dài Phải Từ 5 Trở Lên'
			}
		}
	});
		...
		$.ajax({
			...
			complete: function (res) {
				...
				if(res.status >= 200 && res.status <= 300)
				{
					...
					$(that).trigger('reset');
					$(that).find('.form-control').removeClass('valid');
    ...
    Echo.channel(`product.${idProduct}`)
    .listen('.comment.created',(e) => {
      $(renderComment(e.data,e.name)).prependTo('#mCSB_2_container');
      $("#comments").mCustomScrollbar('scrollTo',[0,0])
    })
...
function renderComment(data,name)
{
	return `<div class="comment">
				<h3>${name}</h3>
				<label>${dateConvert(data['created_at'])}</label>
				<p>
					${data['comment']}
				</p>
			</div>`
